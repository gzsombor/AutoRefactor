...
public class StandardMethodRatherThanLibraryMethodRefactoring extends NewClassImportRefactoring {
    ...
    private boolean replaceBase64(MethodInvocation node, final Set<String> classesToUseWithImport,
        ...
        if (isMethod(node, "org.apache.commons.codec.binary.Base64", "encodeBase64", "byte[]")
            ...
            r.replace(node, b.invoke(b.invoke(base64, "getEncoder"), "encode", b.copy((Expression) node.arguments().get(0))));
            return true;
        }
        if (isMethod(node, "org.apache.commons.codec.binary.Base64", "encodeBase64String", "byte[]")
            ...
            r.replace(node, b.invoke(b.invoke(base64, "getEncoder"), "encodeToString", b.copy((Expression) node.arguments().get(0))));
            return true;
        }
        if (isMethod(node, "org.apache.commons.codec.binary.Base64", "decodeBase64", "java.lang.String")
            ...
            r.replace(node, b.invoke(b.invoke(base64, "getDecoder"), "decode", b.copy((Expression) node.arguments().get(0))));
            return true;
        }
        return false;
    }
